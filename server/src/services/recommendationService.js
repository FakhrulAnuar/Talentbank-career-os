...
function userSignals(userId) {
  const skills = db.select({ title: modules.title })
    .from(userModules).innerJoin(modules, eq(userModules.moduleId, modules.id))
...
function scoreTarget(target, signals, demand) {
  ...
  const check = (title, kind) => {
    const tw = words(title);
    for (const tag of tagList) {
      if (matched.has(tag)) continue;
      const tagWords = tag.split(' ');
      if (tagWords.some((x) => tw.includes(x)) || tw.some((sw) => tag.includes(sw))) {
        matched.add(tag);
        if (kind === 'cert') { certMatched = true; reasons.push(`Your "${title}" certificate fits ${target.field}`); }
        else if (kind === 'interest') { profileMatched = true; reasons.push(`Matches your interest in ${title}`); }
        else if (kind === 'field') { profileMatched = true; reasons.push(`Fits your target field: ${title}`); }
        else reasons.push(`Your ${title} skill aligns with ${tag}`);
      }
    }
  };

  if (signals.targetField) check(signals.targetField, 'field');
  signals.interests.forEach((i) => check(i, 'interest'));
  signals.skills.forEach((s) => check(s, 'skill'));
  signals.certs.forEach((c) => check(c, 'cert'));
...
export function getRecommendations(userId, pathType) {
  const type = pathType === 'university' ? 'company' : 'university';
  const list = listTargets(type);
...
export function topRecommendations(userId, pathType, n = 3) {
  return getRecommendations(userId, pathType).items.slice(0, n)
    .map((r) => ({ id: r.id, label: r.name, score: r.score }));
}
